...
Service.prototype.addRequest = function (req, service) {
  if (!this.getHeader(req, 'x-amino-service')) {
    req._headers || (req._headers = {});
    req._headers['x-amino-service'] = service;
  }
  if (this.specs.length) {
...
Service.prototype.doRequest = function (req) {
  ...
  process.nextTick(function () {
    var spec
      , service = self.getHeader(req, 'x-amino-service')
      , reqSpec = new Spec(service)
      , stickyId = self.getHeader(req, 'x-amino-stickyid')

    if (stickyId) {
      spec = self.specFromRing(stickyId, reqSpec.version);
    }
    else if (reqSpec.version) {
      // Search for a spec to satisfy the request.
      for (var idx in self.specs) {
        if (self.specs[idx].version && semver.satisfies(self.specs[idx].version, reqSpec.version)) {
